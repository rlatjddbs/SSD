...
class HparamEnv(Tap):
    ...
    def read_configs(self, dataset, config, experiment):
        '''
            Load parameters from config file
        '''
        self.dataset = dataset
        dataset = dataset.replace('-', '_')
        print(f'[ utils/setup ] Reading config: {config}:{dataset}')
        
        module = importlib.import_module(config)
        params = getattr(module, 'base')[experiment]

        if hasattr(module, dataset) and experiment in getattr(module, dataset):
            print(f'[ utils/setup ] Using overrides | config: {config} | dataset: {dataset}')
            overrides = getattr(module, dataset)[experiment]
            params.update(overrides)
        else:
            print(f'[ utils/setup ] Not using overrides | config: {config} | dataset: {dataset}')     
    ...
    def hparam_combs(self):
        '''
            Make combination of hparams from config file
        '''
        args = super().parse_args(known_only=True)
        configs = self.read_configs(args.dataset, args.config, args.experiment)
        # params = getattr(module, 'base')[experiment]
        
        env_hparams = []
        for grid_search_value in product(*configs.values()):
            grid_dict = dict(zip(configs.keys(), grid_search_value))
...
class Parser(Tap):
    ...
    def parse_args(self, iterparser):
        args = super().parse_args(known_only=True)
        # args = self.read_config(args, experiment)
        hparam_combs = iterparser.hparam_combs()
        setattr(args, 'dataset', iterparser.dataset)
        setattr(args, 'experiment', iterparser.experiment)
        self.sync_config(args, hparam_combs)
